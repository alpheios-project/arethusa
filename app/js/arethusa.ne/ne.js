...
angular.module('arethusa.ne').service('ne', [
  ...
  function (state, configurator) {
    ...
    function abortOutstandingRequests() {
      if (neRetrievers) {
        angular.forEach(neRetrievers, abortRetriever);
      }
    }
    ...
    function configure() {
      ...
      configurator.getConfAndDelegate('ne', self, props);
      self.analyses = {};
      neRetrievers = configurator.getRetrievers(self.conf.retrievers);
      propagateMappings(neRetrievers);
    }
    ...
    this.getExternalAnalyses = function (analysisObj, id) {
      angular.forEach(neRetrievers, function (retriever, name) {
